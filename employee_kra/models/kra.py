...
class hr_employee(models.Model):
    ...
    def _get_score(self):
        ...
        final_score_last = 0
        #for rec in self:
        kras = self.env['employee.kra'].search([('employee_id', '=', self.id), ('name', '=', str(month)),
        ...
        if kras:
            ...
            self.kra_final_score_this_month = final_score_this

        kras_last = self.env['employee.kra'].search([('employee_id', '=', self.id), ('name', '=', str(last_month)),
                                                ('year', '=', str(year)), ('state', '=', 'done')])
        if kras_last:
            for question_id in kras_last.kra_question_ids:
                final_score_last = final_score_last + question_id.final_score
            self.kra_final_score_last_month = final_score_last
